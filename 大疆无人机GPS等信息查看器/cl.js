...
// 懒加载相关参数
const initialLoadCount = 10; // 初始加载数量
const loadMoreCount = 5; // 每次滚动加载数量
...
function loadPhotos(count) {
  ...
  for (let i = 0; i < loadCount; i++) {
    ...
    const exifItem = createExifItem(file, exifData);
...
// 创建照片项
function createExifItem(file, exifData) {
  ...
  if (exifData) {
    // 检查是否存在十进制度GPS信息
    const hasDecimalCoords = exifData.GPSLatitudeDecimal && exifData.GPSLongitudeDecimal;
    const hasDMScoords = exifData.GPSLatitude && exifData.GPSLongitude;
    
    if (hasDecimalCoords) {
      // 使用现成的十进制度坐标
      gpsInfo = `纬度: ${exifData.GPSLatitudeDecimal.toFixed(6)}<br>经度: ${exifData.GPSLongitudeDecimal.toFixed(6)}`;
  ...
  const dateTaken = exifData.DateTimeOriginal || "Invalid Date";
